...
export default class MBasketballRoster extends Component {
  ...
  getRoster() {
     fetch('https://goatbackend110.appspot.com/static/rosters.json')
        ...
        .then((responseJson) => {
          this.setState({
            size: Object.keys(responseJson.rosters["1"]).length
          })

          for (var i = 0; i < this.state.size; i++) {
            this.setState({
              names: this.state.names.concat([
                responseJson.rosters["1"][i][0]
              ]),
              fullBios: this.state.fullBios.concat([
                responseJson.rosters["1"][i][6]
              ])
            });
          }
        })
...
AppRegistry.registerComponent('MBasketballRoster', () => MBasketballRoster);
