...
export default class MSDRoster extends Component {
  ...
  getRoster() {
     fetch('https://goatbackend110.appspot.com/static/rosters.json')
        ...
        .then((responseJson) => {
          this.setState({
            size: Object.keys(responseJson.rosters["7"]).length
          })

          for (var i = 0; i < this.state.size; i++) {
            this.setState({
              names: this.state.names.concat([
                responseJson.rosters["7"][i][0]
              ]),
              fullBios: this.state.fullBios.concat([
                responseJson.rosters["7"][i][6]
              ])
            });
          }
        })
  ...
  render() {
    ...
    for (let i = 0; i < (this.state.size - (this.state.size % 3)); i += 3) {
      var url1 = "https://goatbackend110.appspot.com/static/rosters/7/" + i + ".png"
      var url2 = "https://goatbackend110.appspot.com/static/rosters/7/" + (i+1) + ".png"
      var url3 = "https://goatbackend110.appspot.com/static/rosters/7/" + (i+2) + ".png"
